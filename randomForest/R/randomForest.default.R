...
"randomForest.default" <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             keep.inbag=FALSE, 
             method = 0,                  # 新增参数：0-原方法，1-新方法
             huangr_mtry = NULL,          # 新增参数：组mtry
             huangr_stratum = NULL,       # 新增参数：层编号
             huangr_start = NULL,         # 新增参数：层起始索引
             huangr_n_vars = NULL,        # 新增参数：层内变量数
             huangr_n_layers = NULL,       # 新增参数：总层数
             ...) {
    addclass <- is.null(y)
    ...
    if (method == 1) {
      if (is.null(huangr_mtry)) {
        if (is.null(huangr_n_layers)) {
          stop("当 method=1 时，必须提供 huangr_n_layers")
        }
        huangr_mtry <- max(floor(huangr_n_layers / 3), 1)  # 总层数/3，最小为1
      }
    }
    ...
    if (classRF) {
        ...
        rfout <- .C("regRF",
                    ...
                    huangr_mtry = as.integer(huangr_mtry),
                    huangr_stratum = as.integer(huangr_stratum),
                    huangr_start = as.integer(huangr_start),
                    huangr_n_vars = as.integer(huangr_n_vars),
                    huangr_n_layers = as.integer(huangr_n_layers),
