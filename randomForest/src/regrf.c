...
void regRF(double *x, double *y, int *useweights, double *weights, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           ...
           int *nout, int *inbag,
           //新增参数
           int *method, int *huangr_mtry, 
           int *huangr_stratum, int *huangr_start,
           int *huangr_n_vars, int *huangr_n_layers) {
            Rprintf("\n[DEBUG] regRF接收到的新增参数:\n");
            Rprintf("----------------------------------------\n");
            ...
            if (huangr_stratum == NULL) {
                ...
                int n_layers = *huangr_n_layers; // 获取实际层数
  ...
  *************************************************************************/
    ...
    for (j = 0; j < *nTree; ++j) {
		...
		regTree(xb, yb, mdim, *sampsize, lDaughter + idx, rDaughter + idx,
                ...
                varUsed,
                //新增参数
                *method, *huangr_mtry, huangr_stratum, huangr_start, huangr_n_vars, *huangr_n_layers);
