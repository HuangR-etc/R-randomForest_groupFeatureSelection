...
void regTree(double *x, double *y, int mdim, int nsample, int *lDaughter,
    ...
    Rprintf("\n[DEBUG] RegTree receive PAR:\n");
    Rprintf("----------------------------------------\n");
    
    // 1. 标量参数
    Rprintf("- method:           %d\n", method);
    Rprintf("- huangr_mtry:      %d\n", huangr_mtry);
    Rprintf("- huangr_n_layers:  %d\n", huangr_n_layers);
    
    // 2. 指针类数组参数（带安全检查）
    Rprintf("\n[数组参数检查]\n");
    
    // huangr_stratum 数组
    if (huangr_stratum == NULL) {
        Rprintf("- huangr_stratum:   NULL 指针\n");
    } else {
        Rprintf("- huangr_stratum 内容 (前 %d 层):\n", huangr_n_layers);
        for (int i = 0; i < huangr_n_layers; i++) {
            Rprintf("  层[%d] = %d\n", i, huangr_stratum[i]);
        }
    }
    
    // huangr_start 数组
    if (huangr_start == NULL) {
        Rprintf("- huangr_start:     NULL 指针\n");
    } else {
        Rprintf("- huangr_start 内容 (前 %d 层):\n", huangr_n_layers);
        for (int i = 0; i < huangr_n_layers; i++) {
            Rprintf("  层[%d] = %d\n", i, huangr_start[i]);
        }
    }
    ...
    // huangr_n_vars 数组
    if (huangr_n_vars == NULL) {
        Rprintf("- huangr_n_vars:    NULL 指针\n");
    } else {
        Rprintf("- huangr_n_vars 内容 (前 %d 层):\n", huangr_n_layers);
        for (int i = 0; i < huangr_n_layers; i++) {
            Rprintf("  层[%d] = %d\n", i, huangr_n_vars[i]);
        }
    }
...
if (method == 0) {
  ...
  findBestSplit_new(x, rowIndices, y, mdim, nsample, ndstart, ndend, &msplit,
                   &decsplit, &ubest, &leftChildLastDataPoint, &splitResult, 
                    sumOfNodeData, currentNodeCount, cat,huangr_mtry,
                   huangr_stratum, huangr_start, huangr_n_vars, huangr_n_layers);
                  }
#ifdef RF_DEBUG
